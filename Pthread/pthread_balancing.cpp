...
void *edge_linking(void *args){
    ...
    queue<int32_t> q_next_iter_point;

    double startTime, endTime;
    startTime = CycleTimer::currentSeconds();
    // printf("edge_link time: %.3f ms\n", (endTime - startTime) * 1000);
    
    // calaulate next iteration pixel need to traversal for this thread
    while(!q_th.empty()){
        index = q_th.front();
        q_th.pop();
        if(visited[index] == 0){
            ...
            // so can be seen as a weak edge pixel connected to an strong edge pixel
                ...
                // up
                if(index - width >= 0)
                    q_next_iter_point.push(index - width);
                // down
                if(index + width < width * height)
                    q_next_iter_point.push(index + width);
                // left
                if(index % width != 0)
                    q_next_iter_point.push(index - 1);
                // right
                if(index % width != width - 1)
                    q_next_iter_point.push(index + 1);
                // up left
                if(index - width - 1 >= 0)
                    q_next_iter_point.push(index - width - 1);
                // up right
                if(index - width + 1 >= 0)
                    q_next_iter_point.push(index - width + 1);
                // down left
    ...
    // write next iteration pixel into share memory
    // collect all thread next iteration pixel
    while(!q_next_iter_point.empty())
    {
        pthread_mutex_lock(&vec_mutex);
        vec.emplace_back(q_next_iter_point.front());
        pthread_mutex_unlock(&vec_mutex);
        q_next_iter_point.pop();
    }
    ...
    while(1){
    	...
    	// if share memory is empty
    	// which mean all pixel is done
        if(vec.size() == 0){
            printf("edge linking done!\n");
            endTime = CycleTimer::currentSeconds();
            // printf("%d edge_link time: %.3f ms\n", thread_id ,(endTime - startTime) * 1000);
            printf("edge linking done!\n");
            pthread_exit(EXIT_SUCCESS);
        }
        ...
        // distribute all pixel to each thread
	...
	// wait all thread get their pixel and clear share memory
        pthread_barrier_wait(&barrier);
        vec.clear();
        pthread_barrier_wait(&barrier);

        queue<int32_t> tmp;
        // start traversal local pixel
        while(!q_th.empty()){
            index = q_th.front();
            q_th.pop();
            if(visited[index] == 0){
	...
	// write next iteration pixel to share memory
        while(!tmp.empty())
        {
            pthread_mutex_lock(&vec_mutex);
            vec.emplace_back(tmp.front());
            pthread_mutex_unlock(&vec_mutex);
            tmp.pop();
        }
        // wait for all thread done and start next iteration
        pthread_barrier_wait(&barrier);
    }
    endTime = CycleTimer::currentSeconds();
    // printf("%d edge_link time: %.3f ms\n", thread_id ,(endTime - startTime) * 1000);
...
int main(){
    ...
    endTime = CycleTimer::currentSeconds();
    printf("edge_linking time: %.3f ms\n", (endTime - startTime) * 1000);
    total_time += (endTime - startTime);
